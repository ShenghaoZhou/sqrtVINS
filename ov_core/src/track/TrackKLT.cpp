...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  PRINT_ALL("[TIME-KLT]: %.4f seconds for pyramid\n",
            std::chrono::duration_cast<std::chrono::microseconds>(rT2 - rT1).count() * 1e-6);
  PRINT_ALL("[TIME-KLT]: %.4f seconds for detection (%zu detected)\n",
            std::chrono::duration_cast<std::chrono::microseconds>(rT3 - rT2).count() * 1e-6,
            (int)pts_last[cam_id].size() - pts_before_detect);
  PRINT_ALL("[TIME-KLT]: %.4f seconds for temporal klt\n",
            std::chrono::duration_cast<std::chrono::microseconds>(rT4 - rT3).count() * 1e-6);
  PRINT_ALL("[TIME-KLT]: %.4f seconds for feature DB update (%d features)\n",
            std::chrono::duration_cast<std::chrono::microseconds>(rT5 - rT4).count() * 1e-6, (int)good_left.size());
  PRINT_ALL("[TIME-KLT]: %.4f seconds for total\n",
            std::chrono::duration_cast<std::chrono::microseconds>(rT5 - rT1).count() * 1e-6);
}
...
void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left,
  ...
  PRINT_ALL("[TIME-KLT]: %.4f seconds for stereo klt\n",
            std::chrono::duration_cast<std::chrono::microseconds>(rT5 - rT4).count() * 1e-6);
  PRINT_ALL("[TIME-KLT]: %.4f seconds for feature DB update (%d features)\n",
            std::chrono::duration_cast<std::chrono::microseconds>(rT6 - rT5).count() * 1e-6, (int)good_left.size());
  PRINT_ALL("[TIME-KLT]: %.4f seconds for total\n",
